...
function start_game() { 
	...
	if (choice == 1) {
		let character_menu = " Character Creation Options\n 1. Custom \n 2. Quick \n"
		...
		if (character_choice == 1) { 
			...
			if (choice_class == 1) {
				...
				const Main = new PlayerCharacter(character_name, 12,18,14,10,15,18);
			...
			const Main = new PlayerCharacter(character_name, 16, 12, 14, 13, 10, 8);
			console.log(`Choose your class:\n 1. Soldier \n 2. Scout \n 3. Scoundrel\n`);
			var choice_class = Number(prompt(``));
			if (choice_class == 1) {
				console.log(`\nYou are ${Main.name}, a former Republic soldier. You find yourself alone on the floor of a medical bay \nwith little recollection of how you got there.`);
...
function fight_menu(pc) {
	...
	if (fight_choice == 1) { 
		console.log(red + 'The call of Korriban is strong but is it the call of the dead. It is fitting that you came here.\n'+ reset);
		const Sion = new Nonplayer_Character('Darth Sion', 16,16,16,16,16.16); 
		return Sion;
	}
	else if (fight_choice == 2){
		console.log(red + 'I am the darkness in which all life  dies' + reset);
		const Nihilus = new Nonplayer_Character('Darth Nihilus', 15, 15, 15, 18, 18, 18);
		return Nihilus; 
	}
	else if (fight_choice == 3) {
		console.log(green + `At last you have arrived.. is Malachor as you remember? You no doubt have many questions.\n I would be a poor teacher if I did not give you the answers you seek here.` + reset);
